...
(defun insert-n (myList number index)
	(if (= index 0) ;;when index value equals to 0 adds number to rest of list and then returns full list.
		(cons number myList)
		(cons (car myList)(insert-n (cdr myList) number (- index 1)))
	)
)
